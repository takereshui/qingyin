...
func EncodeManifest(p ManifestParams) []byte {
	...
	// The platform theme removes title-bar layout space before Activity creation.  The
	// runtime code keeps the same setting as a second line of defence for vendor ROMs.
	e.StartElement("", "application", []Attr{
		...
		AndroidAttr("theme", AttrTheme, RefVal(0x010300b7)), // android:style/Theme.NoTitleBar
	})
