...
当前原生快照对应 **2.0.2-native（versionCode 22）**，显示名称为“轻音”，包名保持 `im.molan.music`，以支持从旧版本覆盖升级。
...
APK 输出在 `native-android/app/build/outputs/apk/debug/app-debug.apk`。原生版已恢复统一音质、封面磁盘缓存、本地歌词匹配缓存、每日推荐、应用内微信/支付宝赞赏，以及 NCMC/ChKSz 私有与备用线路设置。2.0.2 进一步加入沉浸式顶部布局、独立线上搜索、在线播放 URL 回退与网易云登录后的我的歌单自动同步。详见 [`docs/QINGYIN_2.0.2_UI_STREAM_PLAYLISTS.md`](docs/QINGYIN_2.0.2_UI_STREAM_PLAYLISTS.md)。
