...
(async () => {
  ...
  assert.match(native, /completeAuthorizedSession/);
  assert.match(native, /ssl\.ptlogin2\.graph\.qq\.com\/check_sig/);
  assert.match(native, /qqmusic_uin/);
  assert.match(player, /shuffleHistory/);
  assert.match(player, /removeAt/);
  assert.match(app, /openQueueSheet/);
  console.log('PASS: QQ ChKSz mapping, staged QQ authorization, source cache isolation, queue controls, and native session contract');
